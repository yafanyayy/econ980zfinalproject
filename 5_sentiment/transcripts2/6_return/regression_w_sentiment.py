...
# Load the dataset
file_path = "labeled_earningsearch_with_abnormal_beta_1.xlsx"
data = pd.read_excel(file_path)

# For the column drift, it is a list, get the last number
def safe_get_last_value(drift_return):
    if isinstance(drift_return, str):
        try:
            # Safely parse the string as a Python list
            parsed_list = ast.literal_eval(drift_return)
            if isinstance(parsed_list, list) and len(parsed_list) > 0:
                return parsed_list[-1]  # Return the last value in the list
        except (ValueError, SyntaxError):
            return None  # Return None if parsing fails
    return None  # Return None for non-string or invalid values

data['drift_return_75'] = data['drift_return'].apply(safe_get_last_value)
...
sentiment_controls = 'operator_word_count + executives_word_count + analysts_word_count + operator_avg_sentiment + executives_avg_sentiment + analysts_avg_sentiment'
...
def run_regressions(dependent_var, data, specs):
    ...
    for group_var, std_ctrl, vol_ctrl, sorting in specs:
        ...
        if sorting == 'Sentiment':  # Add sentiment controls if specified
            controls.append(sentiment_controls)
...
def create_summary_table(results, file_name):
    ...
    sentiment_ctrl_flags = ['Sentiment' in res[0].model.formula for res in results]  # Check if sentiment controls are included
    ...
    info_dict = {
        ...
        'Sentiment Controls': lambda x: "X" if sentiment_ctrl_flags[models.index(x)] else ""  # Add sentiment control flag
    }
...
create_summary_table(immediate_results, 'immediate_return_sentiment.tex')

# Generate and save Drift Return Table
create_summary_table(drift_results, 'delay_return_sentiment.tex')

# Generate and save Sentiment Table
sentiment_results = run_regressions('immediate_return', data_cleaned, [('top_group', True, True, 'Sentiment')])
create_summary_table(sentiment_results, 'sentiment_regression.tex')